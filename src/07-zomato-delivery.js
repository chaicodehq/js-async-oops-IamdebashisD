...
export function assignRider(order) {
  ...
  let randomIndex = Math.floor(Math.random() * riderPool.length)
...
export function processMultipleOrders(orderList) {
  // Your code here
  const promises = orderList.map( 
    order => processDelivery(order.restaurant, order.items)
  )
