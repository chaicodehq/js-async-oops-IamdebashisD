...
export class Batsman extends Player {
  ...
  playInnings(runs, balls) {
    ...
    const inningsObject = {runs, balls, strikeRate}
    this.innings.push(inningsObject)
  ...
  getStrikeRate() {
    ...
    const totalStrikeRate = this.innings.reduce((total, inning) => {
  ...
  getProfile() {
    // Your code here
    const totalRuns = this.innings.reduce((runs, inning) => 
      runs += inning.runs
    , 0)
    return {
      ...super.getProfile(),
...
export class Bowler extends Player {
  ...
  bowlSpell(wickets, runsConceded, overs) {
    ...
    const spellObject = { wickets, runsConceded, overs, economy }
  ...
  getEconomy() {
    ...
    const totalRunsConceded = this.spells.reduce((economy, spell) => {
      economy += spell.economy
      return economy
    }, 0)

    return totalRunsConceded / this.spells.length
  }

  getProfile() {
    // Your code here
    const totalWickets = this.spells.reduce((sum, spell) => (
      sum += spell.wickets
    ), 0)
    return {
      ...super.getProfile(),
...
export class AllRounder extends Player {
  ...
  getProfile() {
    ...
    const totalWickets = this.spells.reduce((wickets, spell) => 
