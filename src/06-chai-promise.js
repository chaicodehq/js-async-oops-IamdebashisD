...
export function prepareChaiWithTimeout(type, timeoutMs) {
  ...
  const chaiTimeoutPromises = new Promise((_, reject) => {
    timerId = setTimeout(() => (
  ...
  return Promise.race([orderChai(type, 1), chaiTimeoutPromises])
...
export function processChaiQueue(orders) {
  ...
  const eachPromises = orders.map((order) => (
  ...
  return Promise.all(eachPromises)
}
