...
export class Startup {
  ...
  constructor(name, founder, domain) {
    // Your code here
    const validDomain = ['fintech', 'edtech', 'healthtech', 'foodtech']
    if (!validDomain.includes(domain)) throw Error("Invalid domain! Choose from: fintech, edtech, healthtech, foodtech")
...
export class Incubator {
  ...
  constructor(name, maxStartups) {
    // Your code here
    if (maxStartups <= 0) throw new Error("Invalid config!")
    this.name = name
  ...
  async admitStartup(startup) {
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        if (!(startup instanceof Startup)) return reject(new Error("Invalid startup!"));
       
       const exists = this.#startups.some(
        s => s.startup.name === startup.name
       ) 

       if (exists) return reject(new Error("Startup already admitted!"))

       if (this.#startups.length >= this.maxStartups) return reject(new Error("Incubator full!"));
  ...
  async assignMentor(startupName, mentor) {
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        if (!exists) return reject(new Error("Startup not found!"));
  ...
  *[Symbol.iterator]() {
  ...
  static createFromConfig(config) {
    // Your code here
    if (!config?.name || !config?.maxStartups) throw new Error("Invalid config!");
